...
class BaseSolver:
    ...
    def build_optimizer(self):
        ...
        param_groups_i = [
                {'params':self.net.module.INNet.parameters(), 'lr_mult': opt.TRAIN.LR_MULT}
                            ]

        # param_groups_d = [
        #         {'params':self.net.module.domain_classifier.parameters(), 'lr_mult': opt.TRAIN.LR_MULT}
        #                     ]
        ...
        if opt.TRAIN.OPTIMIZER == "Adam":
            ...
            self.optimizer['C'] = optim.Adam(param_groups_c,
                                             lr=self.base_lr, betas=[opt.ADAM.BETA1, opt.ADAM.BETA2],
                                             weight_decay=opt.TRAIN.WEIGHT_DECAY)
            self.optimizer['I'] = optim.Adam(param_groups_i,
                                             lr=self.base_lr, betas=[opt.ADAM.BETA1, opt.ADAM.BETA2],
                                             weight_decay=opt.TRAIN.WEIGHT_DECAY)
            #
            # self.optimizer['D'] = optim.Adam(param_groups_d,
            #                                  lr=self.base_lr, betas=[opt.ADAM.BETA1, opt.ADAM.BETA2],
            #                                  weight_decay=opt.TRAIN.WEIGHT_DECAY)
            ...
            self.lr_scheduler['I'] = self.build_lr_scheduler(self.optimizer['I'], opt.TRAIN)
            # self.lr_scheduler['D'] = self.build_lr_scheduler(self.optimizer['D'], opt.TRAIN)
            ...
            self.optimizer['C'] = optim.SGD(param_groups_c,
                                            lr=self.base_lr, momentum=self.momentum,
                                            weight_decay=opt.TRAIN.WEIGHT_DECAY)
            self.optimizer['I'] = optim.SGD(param_groups_i,
                                            lr=self.base_lr, momentum=self.momentum,
                                            weight_decay=opt.TRAIN.WEIGHT_DECAY)

            # self.optimizer['D'] = optim.SGD(param_groups_d,
            #                                 lr=self.base_lr, momentum=self.momentum,
            #                                 weight_decay=opt.TRAIN.WEIGHT_DECAY)
    ...
    def update_lr(self):
        ...
        if self.opt.TRAIN.LR_SCHEDULE == 'exp':
            ...
            solver_utils.adjust_learning_rate_inv(self.base_lr, self.optimizer['C'],
                                                  iters, self.opt.INV.ALPHA, self.opt.INV.BETA)
            solver_utils.adjust_learning_rate_inv(self.base_lr, self.optimizer['I'],
                                                  iters, self.opt.INV.ALPHA, self.opt.INV.BETA)
            # solver_utils.adjust_learning_rate_inv(self.base_lr, self.optimizer['D'],
            #                                       iters, self.opt.INV.ALPHA, self.opt.INV.BETA)
